...
import std.stdio, std.string, std.array, std.algorithm, std.range, std.math, std.format, std.conv;
...
void process()
{
  ...
  if(cLine.startsWith("#define"))
  {
    if(cLine.endsWith("_GUID \\"))
    {
      ...
      nextLine();
      fpOut.writeln("enum EFI_GUID ",Id," = EFI_GUID(",Guid.replace("{","[").replace("}","]"),");");
    }
    else
    {
      // #define abc def
  ...
  {
    cLine = cLine[8..$];
    if(cLine.startsWith("struct _")&&(cLine[$-1]!='{'))cLine = cLine[7..$];
    if(cLine.startsWith("union _")&&(cLine[$-1]!='{'))cLine = cLine[6..$];
    if(cLine.startsWith("struct"))
    {
      ...
      string Id = cLine[2..$-1].idup;
      fpOut.writeln("struct ", Id, " {\n", app.data, "}");
    }
  ...
  if(cLine=="typedef") // function pointer
  {
    ...
    while(cLine!=");")
    {
      ...
      foreach(K;Kw)
      {
        if((K=="IN")||(K=="OUT")||(K=="EFIAPI")||(K=="OPTIONAL"))continue;
        if((K=="IN,")||(K=="OUT,")||(K=="EFIAPI,")||(K=="OPTIONAL,")){Args~=',';continue;}
        if(K=="CONST")K="const".dup;
        if(K=="VOID")K="void".dup;
        Args ~= K;
        Args ~= ' ';
      }
      nextLine();
    }
    fpOut.writefln("alias %s = %s function(%s) @nogc nothrow;", FunName, RetType, Args.data);
  }
  if(cLine.startsWith("struct"))
  {
